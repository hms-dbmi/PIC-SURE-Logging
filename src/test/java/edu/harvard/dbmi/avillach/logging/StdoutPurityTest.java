...
/**
 * Stdout is the audit-JSON channel that Splunk parses. Nothing but JSON may appear there.
 * The Spring banner prints to System.out before logging initializes, which is why
 * spring.main.banner-mode must be off.
 */
class StdoutPurityTest {
    ...
    void startupWritesNothingNonJsonToStdout() {
        ...
        List<String> lines = captured.toString(StandardCharsets.UTF_8)
            .lines()
            .filter(line -> !line.isBlank())
            .toList();

        assertThat(lines)
            .withFailMessage("stdout must carry only audit JSON, but got:%n%s", String.join("\n", lines))
