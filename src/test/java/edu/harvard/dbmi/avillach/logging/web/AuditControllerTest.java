...
class AuditControllerTest {
    ...
    /**
     * An empty body raises HttpMessageNotReadableException before the controller runs.
     * Javalin returned 400 here (readValue("") threw). A bare @ExceptionHandler(Exception.class)
     * catch-all would turn it into 500 — this pins the contract.
     */
    @Test
    ...
    void wrongHttpMethodIsNotAServerError() {
        ...
        ResponseEntity<String> response =
            rest.exchange("/audit", HttpMethod.GET, new HttpEntity<>(headers), String.class);
    ...
    void unknownFieldsAreIgnored() {
        assertThat(postAudit("{\"event_type\":\"TEST\",\"unknown_field\":\"v\"}", API_KEY)
    ...
    void missingContentTypeStillReturns202NotUnsupportedMediaType() {
        ...
        ResponseEntity<String> response = rest.exchange(
            ...
            new HttpEntity<>("{\"event_type\":\"TEST\"}", headers), String.class);
    ...
    void nestingBeyondDepth10Returns400() {
        String json = "{\"event_type\":\"TEST\",\"metadata\":"
            + "{\"k\":".repeat(15) + "\"v\"" + "}".repeat(15) + "}";
    ...
    void metadataOver50KeysReturns400() {
        assertThat(postAudit("{\"event_type\":\"TEST\",\"metadata\":" + jsonMap(51) + "}", API_KEY)
            .getStatusCode().value()).isEqualTo(400);
    }

    @Test
    void metadataAtExactly50KeysReturns202() {
        assertThat(postAudit("{\"event_type\":\"TEST\",\"metadata\":" + jsonMap(50) + "}", API_KEY)
            .getStatusCode().value()).isEqualTo(202);
    }

    @Test
    void errorOver20KeysReturns400() {
        assertThat(postAudit("{\"event_type\":\"TEST\",\"error\":" + jsonMap(21) + "}", API_KEY)
            .getStatusCode().value()).isEqualTo(400);
    }

    @Test
    void errorAtExactly20KeysReturns202() {
        assertThat(postAudit("{\"event_type\":\"TEST\",\"error\":" + jsonMap(20) + "}", API_KEY)
            .getStatusCode().value()).isEqualTo(202);
    }
    ...
    /**
     * A chunked request declares no Content-Length, so RequestSizeLimitFilter cannot reject it
     * up front. The counting wrapper trips mid-read and throws RequestBodyTooLargeException,
     * which ApiExceptionHandler maps to 413.
     *
     * This is the test that proves the exception must be unchecked: an IOException here would be
     * rewrapped by Spring's argument resolver as HttpMessageNotReadableException and return 400.
     * TestRestTemplate always sets Content-Length, so this uses the JDK client, whose
    ...
    void chunkedBodyOverTheCapReturns413() throws Exception {
        ...
        byte[] payload = ("{\"event_type\":\"TEST\",\"action\":\"" + "x".repeat(1_048_576) + "\"}")
            .getBytes(StandardCharsets.UTF_8);

        HttpRequest request = HttpRequest.newBuilder(URI.create("http://localhost:" + port + "/audit"))
    ...
    private static String jsonMap(int keys) {
        return IntStream.rangeClosed(1, keys)
