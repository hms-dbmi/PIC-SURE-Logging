 ...
 * would swallow Spring MVC's own exceptions and turn them into 500s. The one that matters:
 * an empty request body raises HttpMessageNotReadableException before the controller runs,
 * and the frozen contract requires 400 there, not 500. The base class maps it correctly.
 */
...
public class ApiExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    /**
     * Raised by the counting wrapper on a chunked body that passes the 1 MB cap. The
     * declared-Content-Length case never reaches here — RequestSizeLimitFilter writes
     * that 413 itself, outside DispatcherServlet.
     */
    @ExceptionHandler(RequestBodyTooLargeException.class)
    public ResponseEntity<Map<String, String>> handleTooLarge(RequestBodyTooLargeException e) {
        return ResponseEntity.status(HttpStatus.PAYLOAD_TOO_LARGE)
            .body(Map.of("status", "error", "message", "Request body too large"));
    }
