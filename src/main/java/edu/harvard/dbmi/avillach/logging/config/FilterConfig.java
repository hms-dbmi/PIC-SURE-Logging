...
/**
 * Filter ordering across the app:
 *   0  CorsFilter (in WebConfig) — answers preflight and short-circuits it
 *   1  ApiKeyAuthFilter          — 401
 *   2  RequestSizeLimitFilter    — 413
 */
@Configuration
public class FilterConfig {

    /**
     * Order 1: authentication precedes the size check, matching Javalin, where
     * app.before("/audit") ran before ctx.body() ever triggered the maxRequestSize
     * check. An unauthenticated oversized request therefore gets 401, not 413.
     */
    @Bean
